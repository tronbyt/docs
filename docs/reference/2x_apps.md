...
!!! note
    ...
    `pixlet render -2` will automatically add a `@2x` suffix to the filename if none is specified.
...
!!! note
    ...
    Not all fonts have alternatives at exactly 2x the size. Sometimes, one needs to choose a font with double the height, in other cases, double the width is better. Use `pixlet community list-fonts` to see available fonts and their dimensions.
