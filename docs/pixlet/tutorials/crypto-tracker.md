...
us access to Pixlet's [full set of Widgets](../reference/widgets.md). Next is the
...
and the icon simultaneously, we'll use the `Row` Widget to have them
...
Don't worry if all this alignment stuff feels a bit confusing at
...
repeatedly reload our browser (pointed at `http://localhost:8080/`), we
