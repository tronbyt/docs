...
Each Tronbyt regularly sends heartbeats to the Tronbyt cloud, announcing what it has cached. The Tronbyt cloud decides which app needs to be rendered next, and executes the appropriate Starlark script. It encodes the result as a [WebP](https://developers.google.com/speed/webp) image, and sends it back to the device.
...
When apps are published to the [Tronbyt apps repo][3], users can install and configure them with the Tronbyt app. [Define a schema for your app][4] to enable this.
...
when making a request, and the HTTP response will be cached
